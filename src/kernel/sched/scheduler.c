...
// Task entry wrapper — when a task function returns, mark it DEAD
static void task_entry_wrapper(void (*entry)(void)) {
    entry();
    // Task returned — mark as dead
    if (current_task_id >= 0) {
        tasks[current_task_id].state = TASK_DEAD;
    }
    // Yield forever (scheduler will skip dead tasks)
    while (1) {
        scheduler_yield();
    }
}
